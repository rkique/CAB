...
beforeAll((done) => {
  ...
  try {
    ...
    console.log('beforeAll: groups populated');


    const startNodes = (cb) => {
      console.log('startNodes: spawning n1');
      distribution.local.status.spawn(n1, (e, v) => {
        console.log('startNodes: n1 callback', e, v);
        if (e) {
          done(e);
          return;
        }
    
        console.log('startNodes: spawning n2');
        distribution.local.status.spawn(n2, (e, v) => {
          console.log('startNodes: n2 callback', e, v);
          if (e) {
            done(e);
            return;
          }
    
          console.log('startNodes: spawning n3');
          distribution.local.status.spawn(n3, (e, v) => {
            console.log('startNodes: n3 callback', e, v);
            if (e) {
              done(e);
              return;
            }
    
            console.log('startNodes: all spawned');
            cb();
          });
        });
      });
    };
    console.log('beforeAll: calling distribution.node.start');
distribution.node.start((e) => {
  console.log('beforeAll: distribution.node.start callback', e);
  if (e) {
    done(e);
    return;
  }
  ...
  console.log('beforeAll: calling startNodes');

  startNodes(() => {
    console.log('beforeAll: startNodes callback');

    distribution.local.groups.put(ncdcConfig, ncdcGroup, (e, v) => {
      console.log('beforeAll: local.groups.put ncdc callback', e, v);

      distribution.ncdc.groups.put(ncdcConfig, ncdcGroup, (e, v) => {
        console.log('beforeAll: ncdc.groups.put callback', e, v);

        const avgwrdlConfig = {gid: 'avgwrdl'};
        distribution.local.groups.put(avgwrdlConfig, avgwrdlGroup, (e, v) => {
          console.log('beforeAll: local.groups.put avgwrdl callback', e, v);

          distribution.avgwrdl.groups.put(avgwrdlConfig, avgwrdlGroup, (e, v) => {
            console.log('beforeAll: avgwrdl.groups.put callback', e, v);

            const cfreqConfig = {gid: 'cfreq'};
            distribution.local.groups.put(cfreqConfig, cfreqGroup, (e, v) => {
              console.log('beforeAll: local.groups.put cfreq callback', e, v);

              distribution.cfreq.groups.put(cfreqConfig, cfreqGroup, (e, v) => {
                console.log('beforeAll: cfreq.groups.put callback', e, v);
