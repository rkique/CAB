...
const reducer = (key, values) => {
  const totalDocs = 3;
  const docsWithTerm = values.length;
  ...
  values.forEach((v) => {
    ...
    scores[v.doc] = parseFloat((tf * idf).toFixed(2));
  });
...
test('(10 pts) (scenario) all.mr:strmatch', (done) => {
  ...
  const dataset = [
    ...
    {'doc2': 'Distributed systems are fun to study'},
    {'doc3': 'A fox is a clever animal'},
    {'doc4': 'Graphs and indexes are useful'},
    {'doc5': 'My dog likes treats'},
  ];

  const expected = [
    {'matches': expect.arrayContaining(['doc1', 'doc3', 'doc5'])},
  ];
