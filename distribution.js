...
function bootstrap(config) {
  ...
  Object.defineProperty(distribution, "__setup", {
    value: setup,
    enumerable: false,
  });
...
if (require.main === module) {
  ...
  if (idx !== -1 && idx + 1 < process.argv.length) {
    // @ts-ignore Optional dependency for reference implementation.
    const refLib = require('@brown-ds/distribution')();
    config = refLib.util.deserialize(process.argv[idx + 1]);
  }

  globalThis.distribution = distribution(config);
  globalThis.distribution.node.start(
      globalThis.distribution.node.config.onStart || (() => {}),
  );
}
